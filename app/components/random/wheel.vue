...
function rollForItem(target: number | undefined = undefined): number | undefined {
    ...
    // compute total distance in item-units (not degrees)
    let totalDistance = randomFloat - startOffset + fullSpins * len.value
    if (totalDistance < minItemsPassed) {
        const needed = Math.ceil((minItemsPassed - (randomFloat - startActualOffset)) / len.value)
        const adjusted = Math.max(fullSpins, needed)
        totalDistance = randomFloat - startOffset + adjusted * len.value
    }
    ...
    if (spinDuration <= 0) {
        ...
        function spinAnimate(now: number) {
            ...
            // keep fractional value during animation
            spinningIndex.value = startOffset + totalDistance * eased

            if (t < 1) {
                requestAnimationFrame(spinAnimate)
            } else {
                spinningIndex.value = spinningIndex.value % props.list.length
                rolling.value = false
                emit('select', finalIndex)
            }
        }
...
<template>
    <div :style="{ width: SIZE + 'px', height: SIZE + 'px' }" class="relative select-none rotate-90 pointer-events-none">
        <!-- Wheel container -->
        <div
        ...
        >
            <!-- Rotating group -->
            <div
                class="absolute inset-0 flex items-center justify-center will-change-transform transform-gpu"
                :style="rotationStyle"
            >
            ...
            <!-- pointer at top -->
            <div
                :style="{ left: (SIZE / 2 - 8) + 'px', top: '-8px', rotate: '180deg' }"
